...
후속: 사용자의 시도 승인으로 [0.2.3-visual-test 화면 분석 보조](VISUAL_ASSIST_TRIAL.md)를 구현·시험 중이다. 아래0.2.2/C차 결과는 이전 단계의 증거이며, 새 버전의 성공 결과가 아니다.

## 후속 질문 · 음악·배경음 반복 비교 (18:51 이후)

20:15 후속확인: 사용자OS승인후별도시험앱에서 **Instagram 내부PCM양성수신성공**. IG자체음소거상태0→해제·재생후최대-11.07dBFS/peak17222.60초종료2회확인. [세션수치·원상복원·남은한계](AUDIO_PROBE_TRIAL.md#기기-수신-시험--device-capture-test). 아래미실행/승인대기는과거이력. 반복주기나자동넘김판정은여전히미구현이다.

후속 승인: 사용자가 “그래 해봐”로 수신 시험 승인. 기존 앱과 별개인 [audio-probe](AUDIO_PROBE_TRIAL.md)를 구현·검증 중. 아래 승인질문은 당시 이력이며 현재 실제 OS 동의·PCM 수신은 대기 상태다.

사용자가 화면보다 소리 반복 비교가 쉬운지 질문했다. **가능성 있는 별도 보조 방법이며 아직 오디오 캡처/권한 추가/녹음 시험을 하지 않았다.**
...
- [MediaProjection 실행 조건](https://developer.android.com/media/grow/media-projection): 포그라운드서비스 선언/실행 상태 알림과 세션 동의·종료 처리가 필요하다. 현재 접근성 창캡처와 별개이므로 새 권한/승인은 추가 확인 대상이다.
...
- 현재는 사용자에게 내부재생음 시험을 위한 새권한/승인창 적용 여부를 질문하는 단계. 제품 마이크/인터넷 권한은 추가하지 않았다.

2026-08-27, ShortsLoop0.2.2/code9. 최신 요청은 광범위한 대안/반복시험과 **20개 이상 실제 자동 진행 성공 후에만 새 빌드 Public 게시**다. 제품 수정·새 권한·앱 설치·외부 게시는 하지 않았다. 화면 반복 후보는 찾았지만 **정확한 N회 반복용 대체 감지기는 아직 미구현이며20개 연속 조건도 미달**이다.
...
- Android 공식 [AccessibilityService](https://developer.android.com/reference/android/accessibilityservice/AccessibilityService) / [AccessibilityServiceInfo](https://developer.android.com/reference/android/accessibilityservice/AccessibilityServiceInfo): 앱의 화면 캡처는 `canTakeScreenshot` capability 선언이 필요하며 일반 캡처는API30+, 창 캡처는API34+다. 현재 제품에는 이 capability가 없다. 창별 캡처는 오버레이 혼입을 줄이는 후속 후보이며 현재 shell 프로브와 API 경로가 다르다. **사용자에게 새 capability·시험 APK 설치를 질문했으며 승인 전 제품에 추가하지 않는다.**
- 현재 검색기는 최소약2.1주기를 보고 후보를 내므로 처음 진입한 영상에서N1/N2를 그대로 보장하지 못한다. 주기를 알아도 실제0초 위치를 모를 수 있고 영상 내부 반복 장면은 전체 길이의 절반 같은 가짜 후보를 만든다. 학습 완료 전의 재생을 소급하여 즉시 넘기는 방식도 기존N회 의미와 다르다. 보조 모드의 준비 구간/추가 재생/불확실 상태를 명시하고 사용자와 계약을 정해야 한다.
...
- 최종 실행ON·기준/현재목표1·두앱선택·광고/플로팅ON·blocked=false 유지, 멈춘 장면 그대로 두었다. 새로운 제품 빌드/권한/설치/게시/메일은 없다.
...
The current search needs about2.1 periods of observation and cannot always distinguish internal repeated motion from a full video loop. Product screenshot capability, explicit opt-in semantics, safety gates and test installation need approval before integration. Audio capture was researched but not attempted. After restoring the unchanged app, one normal video advanced automatically and the next stalled at0/1 without playback information. **The20-consecutive requirement failed. No new product build, installation or public release was made.**
...
- 공개 Chrome 확장 [Instagram-Auto-Scroller 소스](https://github.com/sagerkudrick/Instagram-Auto-Scroller/blob/main/content.js)는 웹 video의 loop 속성을 제거하고 ended 이벤트에 다음 이동을 연결한다. 웹 영상 객체에 접근하는 구현이므로 일반 Android Instagram 앱의 접근성 reader에 그대로 적용할 수 없다. 브라우저 시청 전환은 현재 사용 요구와 다르다.
...
- 제품 코드/권한/설치 변경 없음. 제품 전체 빌드·JUnit·lint는 재실행하지 않음. 조사 도구 컴파일/실기기 숫자 출력/문서 링크·diff 검사를 적용. 게시·메일 없음.
