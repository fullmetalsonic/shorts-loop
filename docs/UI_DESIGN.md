...
아래는 현재 네이티브 구현 기준이며 모든 화면 크기·큰 글꼴·최신 빌드 전체 UX의 PASS 선언이 아니다. 과거0.2.0 입력·플로팅과v021b 광고 동작은 이력으로 유지한다. 특히 당시 ‘0회 광고 중지’ 시험은 새 광고 독립 동작을 검증한 것이 아니다. 최종 시험·기기 결과는 [검증 기록](VERIFICATION.md)과 [0.2.4 릴리스](releases/v0.2.4.md), 동작 계약은 [PRODUCT_SPEC](PRODUCT_SPEC.md), 조작 순서는 [USER_GUIDE](USER_GUIDE.md)를 따른다.
...
사용자가 하단 실행 영역의 목적을 질문했다. 고정 이유는 설정 화면 어디서든 현재 상태를 보고 끌 수 있게 하는 것이다. 과거에는 높이 축소·명칭 변경을 제안만 했으나 0.2.4의 기능별 분류 승인으로 명칭을 ‘전체 자동 넘김 실행’으로 바꿨다. 이 명칭 변경을 높이 축소나 모든 화면의 사용성 검증 완료로 확대하지 않는다.
...
색상만으로 켜짐/꺼짐을 구별하지 않는다. 체크/라디오/스위치의 모양과 텍스트 상태를 함께 사용한다. 0회에서는 광고 선택에 따라 ‘광고만 넘김 / 일반·시간제 중지’ 또는 ‘일반·시간제 중지 / 광고 꺼짐’을 표시한다. 빠른 설정은 ‘0회 · 광고만 넘김’과 ‘0회 · 넘김 대기’를 구분한다. 전체 OFF만 모든 동작 중지임을 명확히 한다.
...
- 초 초안은 상태 갱신으로 덮어쓰지 않는다. 완료/‘입력한 시간 적용’/Enter를 지원한다. 시간제 ON은 유효한 초 입력이 필요하며, 전체 실행 ON은 N>0·Instagram 선택·시간제 ON인 경우에만 초 초안을 검증한다. 광고 전용 0회 사용을 무관한 초 오류로 막지 않는다.
...
정상 영상은 C/M, 시간제는 남은 ‘초’, 0회 광고 전용은 ‘광고’를 표시해 같은 숫자 모양으로 다른 의미를 숨기지 않는다. 터치 방식 이름은 ‘횟수 순환’과 ‘반복 켜기·끄기 · 광고는 별도’다. 0↔N은 일반 반복·시간제만 조절하며 광고까지 멈추려면 ×/전체 실행/타일 OFF를 사용한다.
...
- 화면은 Java/native Android로 직접 구현한다. `UiTheme`는 색상·카드·여백, `SettingsScreen`은 배치, `CountEditor`는 횟수, `SecondsEditor`는 초, `MainActivity`는 설정/상태·동의 연결을 담당한다.
...
사용자는 정상·시간제·광고 합계20연속 검증이 끝나기 전에 현재 시험판 공개를 승인했다. 미완료를 PASS로 바꾸지 않는다. 미연결 `VisualSequenceTracker` 20시험 중2실패는 원본을 보존하고 제품 앱·제품 시험에서 제외한다. 기존 선택형 화면 분석을 제거했다는 뜻도, 제품 시간제가 실패했다는 뜻도 아니다. 별도 오디오 시험 APK는 제품 배포에 첨부하지 않는다. 최종 시험 수·해시·CI·기기 증거는 [릴리스 기록](releases/v0.2.4.md)에서 관리한다.
