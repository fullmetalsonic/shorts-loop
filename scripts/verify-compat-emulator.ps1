...
$taskMain = if ($Apk) { (Resolve-Path -LiteralPath $Apk).Path } else { Join-Path $taskRoot 'app/build/outputs/apk/debug/app-debug.apk' }
...
$taskResult = (& $taskAdb -s $Device shell am instrument -w -e expectRelease $ExpectRelease.ToString().ToLowerInvariant() com.fullmetalsonic.shortsloop.test/com.fullmetalsonic.shortsloop.CompatibilityInstrumentation 2>&1 | Out-String)
