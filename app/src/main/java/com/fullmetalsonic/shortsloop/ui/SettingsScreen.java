...
public final class SettingsScreen {
    ...
    public SettingsScreen(Context c, int initial, CountEditor.Listener countListener, int initialSeconds,
        ...
        names.addView(UiTheme.text(c, "반복 · 길이 · 시간제 · 광고 · 라이브", 13, UiTheme.MUTED, false));
        ...
        adsCard.addView(UiTheme.text(c, "광고만 넘기려면: 반복 0회 + 광고 켜기 + 긴 영상·라이브 끄기 + 전체 실행 켜기", 13, UiTheme.TEXT, false));
        ...
        quick = radio(c, "반복 켜기·끄기\n0 ↔ 기준 횟수 · 긴 영상·광고·라이브는 별도", R.id.tap_quick);
        tapModes.addView(rotary); tapModes.addView(quick); floatingDetails.addView(tapModes);
        floatingDetails.addView(UiTheme.text(c, "0회는 반복·시간제만 중지합니다. 긴 영상 옵션이 켜지면 ‘조건’, 광고·라이브만 켜지면 ‘광고’·‘라이브’·‘광·라’로 표시합니다.\n숫자를 끌면 이동 · 모서리 ×는 모든 자동 넘김 종료", 13, UiTheme.MUTED, false)); floatCard.addView(floatingDetails);
        ...
        TextView details = UiTheme.text(c,
                ...
                + "\n\n0회는 일반 영상의 반복·시간제 넘김을 멈춥니다. 긴 영상·광고·라이브는 반복 횟수와 별도입니다. 아래 전체 실행을 끄면 모두 멈추고 플로팅도 닫힙니다."
                + "\n\n긴 영상 건너뛰기는 선택한 YouTube·Instagram 일반 영상의 총길이가 기준 이상일 때 재생 처음이나 완주를 기다리지 않고 안전 확인 후 넘깁니다. 기본 꺼짐·기준 60초이며 1~3600초로 조절합니다. 대기 시간이 아니라 총길이 기준입니다. 길이를 읽을 수 없으면 추측하지 않고, 진행 정보 없는 Instagram은 기존 시간제 설정을 따릅니다. 옵션과 전체 실행이 켜져 있으면 반복 0회에서도 긴 영상을 넘기지만 시간제는 0회에서 멈춥니다."
                ...
                + "\n\n시간제 넘김은 진행 정보를 읽을 수 없는 Instagram 영상에만 적용합니다. 기본은 꺼짐이며 초기 10초를 5~60초로 조절합니다. 정확한 완주 횟수가 아닌 대기 시간이고 정지·댓글·다른 화면에서 돌아오면 처음부터 셉니다. 정상 진행 정보가 있고 긴 영상 건너뛰기 조건에 해당하지 않으면 기존 반복 횟수를 따릅니다."
        ...
        execution = toggle(c, "", R.id.execution_toggle); execution.setContentDescription("반복·긴 영상·시간제·광고·라이브 전체 실행 켜기 또는 끄기");
