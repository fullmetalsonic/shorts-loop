...
public final class SettingsScreen {
    ...
    public SettingsScreen(Context c, int initial, CountEditor.Listener countListener, int initialSeconds, SecondsEditor.Listener secondsListener) {
        ...
        names.addView(UiTheme.text(c, "반복 · 시간제 · 광고 넘김을 한곳에서", 13, UiTheme.MUTED, false));
        ...
        adsCard.addView(UiTheme.text(c, "광고만 넘기려면: 반복 0회 + 광고 켜기 + 전체 실행 켜기", 13, UiTheme.TEXT, false));
        ...
        quick = radio(c, "반복 켜기·끄기\n0 ↔ 기준 횟수 · 광고는 별도", R.id.tap_quick);
        tapModes.addView(rotary); tapModes.addView(quick); floatingDetails.addView(tapModes);
        floatingDetails.addView(UiTheme.text(c, "0은 반복·시간제만 중지 · 광고만 켜져 있으면 ‘광고’ 표시\n숫자를 끌면 이동 · 모서리 ×는 전체 실행 종료", 13, UiTheme.MUTED, false)); floatCard.addView(floatingDetails);
        ...
        TextView details = UiTheme.text(c,
                ...
                + "\n\n0회는 일반 영상의 반복·시간제 넘김을 멈춥니다. 광고는 반복 횟수와 별도입니다. 아래 전체 실행을 끄면 광고까지 모두 멈추고 플로팅도 닫힙니다."
                ...
                + "\n\n인터넷 권한 없음 · 영상·계정·시청 이력 저장 없음. 서비스가 재연결되면 안전을 위해 꺼진 상태로 시작합니다.",
        ...
        execution = toggle(c, "", R.id.execution_toggle); execution.setContentDescription("반복·시간제·광고 전체 실행 켜기 또는 끄기");
