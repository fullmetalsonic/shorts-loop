...
final class PhotoServiceChecks {
    static int run(Context context, SettingsStore store) {
        ...
        try {
            ...
            store.photoMode(1);store.photoFallback(false);store.photoWholeSeconds(5);store.photoSlideSeconds(5);RuntimeState.blocked=false;
            ...
            observe(service,known,0);require(RuntimeState.status.equals("photo.each")&&RuntimeState.current==0,"Photo display is not a counted play");checks++;
            ...
            observe(service,missing,300);require(RuntimeState.status.equals("photo.index_missing"),"Missing-index OFF waits");checks++;
            store.photoFallback(true);observe(service,missing,600);require(RuntimeState.status.equals("photo.fallback"),"Missing-index ON uses dedicated fallback");checks++;
            store.photoEnabled(false);observe(service,known,900);require(RuntimeState.status.equals("photo.disabled"),"Independent switch stops photo timer");checks++;
            ...
            require(RuntimeState.blocked&&!transition.pending(),"In-flight interruption hard-stops");checks++;
            ...
            require(RuntimeState.blocked&&(Boolean)get(service,"unresolvedPhotoAttempt"),"Changing delay cannot rearm failed photo request");checks++;
            ...
            store.enabled(false);RuntimeState.blocked=false;return checks;
