...
- Dual mode is opt-in/default OFF. OFF processes only the active host window; ON observes both selected visible non-PiP hosts with fully independent counters/settings/failures. Never force playback or blindly alternate focus. Serialize OS gestures and semantic scroll actions through immutable leases; expired or changed-page deferred intentions are discarded and fresh geometry is revalidated before dispatch. Preserve the original settings keys and migrate each host once.
