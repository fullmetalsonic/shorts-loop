...
- Timer fallback is Instagram-only for eligible clockless single videos: default OFF, initial3 seconds for unset values, range2–60. Preserve existing valid saved values and toggle. The two-second qualification is included in the selected total, not added afterward. It is a timeout, not a precise playback count.
- Instagram ad pre-skip delay is separate from its ad toggle and ordinary count: integer tenths0–99, default0, displayed0.0–9.9 seconds with0.1-second steps. Zero means no extra delay after safety guards, not disabled. Positive delay requires stable same-ad source identity, expires/cancels on unsafe or changed page/window/settings/execution state, and never occupies the shared input lease while waiting. Unknown identity waits; delay is not a substitute for transition confirmation.
- TikTok support is limited to com.ss.android.ugc.trill and readable normalized progress on ordinary recommendation-feed videos. A0–10000 range is not seconds; never fabricate duration or reuse duration-based long-video filtering. Clockless timing, ad/LIVE/photo filtering and other variants are unsupported. Unknown/unconfirmed destinations fail closed and may require explicit execution OFF/ON. Keep pause, interaction, page/window identity and transition proof separate from progress. Read-only46.7.3 observations are not physical product automation PASS.
